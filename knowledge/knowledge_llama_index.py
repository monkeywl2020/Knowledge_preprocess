...
from llama_index.core import SimpleDirectoryReader,wlSimplePdfDirectoryReader
from llama_index.core.embeddings.wl_custom_embeding import wlMultiModalEmbedding
from llama_index.core.indices.multi_modal.wl_multi_vectorstoreindex import wlMultiModalVectorStoreIndex

# 文档切块配置
# llama-index的设置为1600，实际中文含标点符号，智能 1238个字左右，1500的时候这个1238还会分片，所以两个文档汉字个数控制在 1250左右，最好1100以内
...
class LlamaIndexKnowledge(Knowledge):
    ...
    def _data_to_docs(
        ...
        # 如果loader是 SimpleDirectoryReader 就不用persist_root参数
        if isinstance(loader, SimpleDirectoryReader):
            # this is for loading a document directory
            # 这个是 解析文件的配置，这里面可以指定 解析文件的类型，以及 解析文件的路径
            if query is None:
        ...
        # 如果loader是 wlSimplePdfDirectoryReader 就可以使用persist_root参数
        elif isinstance(loader, wlSimplePdfDirectoryReader):
            if query is None:
    ...
    def _set_transformations(self, config: dict) -> Any:
        ...
        if "store_and_index" in config:
            ...
            if data_processing:
                ...
                image_desc_file = load_data.get("img_desc", None)  # 这个是获取图片文件的描述文件
                logger.info(f"image_desc_file:{image_desc_file}")
                
                # 这个图片描述的关键格式化格式字符串模版，可配置的
                img_desc_format_str = load_data.get("img_desc_format_str", {}) #这个是获取图片文件的描述文件的格式化字符串
            else:
            ...
            if image_desc_file is None:
                transformations = [
                    SentenceSplitter( 
                        chunk_size = self.knowledge_config.get("chunk_size",DEFAULT_CHUNK_SIZE ),
                        chunk_overlap = self.knowledge_config.get("chunk_overlap",DEFAULT_CHUNK_OVERLAP),
                    )
                ]
            else:
    ...
    def _get_retriever(
        ...
        **kwargs: Any,
        ...
        retriever = self.index.as_retriever(
            ...
            image_similarity_top_k = similarity_top_k or DEFAULT_TOP_K,
            **kwargs,
        )
